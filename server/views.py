    ...
    try:
        ...
        for item in result:
            ...
            if ret_item is not None:
                sorted_json_array = sorted(ret_item['list'],
                                           key=lambda x: datetime.strptime(x['publish_time'], '%Y-%m-%d %H:%M:%S.%f'), reverse=True)
                ret_item['list'] = sorted_json_array
                for it in ret_item['list']:
